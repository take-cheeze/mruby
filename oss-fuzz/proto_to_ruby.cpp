...
std::string
protoConverter::removeSpecial(const std::string &x)
{
  std::string tmp(x);
  if (!tmp.empty())
...
void
protoConverter::visit(ArrType const &x)
{
...
void
protoConverter::visit(Array const &x)
{
  switch (x.arr_func()) {
  case Array::FLATTEN:
    visit(x.arr_arg());
...
void
protoConverter::visit(AssignmentStatement const &x)
{
...
void
protoConverter::visit(BinaryOp const &x)
{
  m_output << "(";
  visit(x.left());
  switch (x.op()) {
...
void
protoConverter::visit(BuiltinFuncs const &x)
{
...
void
protoConverter::visit(Const const &x)
{
...
void
protoConverter::visit(Function const &x)
{
  m_output << "def foo()\nvar_0 = 1\n";
  visit(x.statements());
  m_output << "end\n";
...
void
protoConverter::visit(HashType const &x)
{
  if (x.keyval_size() > 0) {
    int i = x.keyval_size();
    m_output << "{";
...
void
protoConverter::visit(IfElse const &x)
{
  m_output << "if ";
  visit(x.cond());
  m_output << "\n";
...
void
protoConverter::visit(KVPair const &x)
{
...
void
protoConverter::visit(MathConst const &x)
{
  switch (x.math_const()) {
  case MathConst::PI:
    m_output << "Math::PI";
...
void
protoConverter::visit(MathOps const &x)
{
  switch (x.math_op()) {
  case MathOps::CBRT:
    m_output << "Math.cbrt(";
...
void
protoConverter::visit(MathType const &x)
{
...
void
protoConverter::visit(ObjectSpace const &x)
{
  switch (x.os_func()) {
...
void
protoConverter::visit(Rvalue const &x)
{
...
void
protoConverter::visit(Statement const &x)
{
...
void
protoConverter::visit(StatementSeq const &x)
{
  if (x.statements_size() > 0) {
    m_numVarsPerScope.push(0);
    m_output << "@scope ||= begin\n";
    for (auto &st : x.statements()) visit(st);
    m_output << "end\n";
...
void
protoConverter::visit(StringExtNoArg const &x)
{
...
void
protoConverter::visit(Ternary const &x)
{
  m_output << "(";
  visit(x.tern_cond());
  m_output << " ? ";
...
void
protoConverter::visit(Time const &x)
{
  switch (x.t_func()) {
  case Time::AT:
    m_output << "Time.at";
...
void
protoConverter::visit(VarRef const &x)
{
  m_output << "var_" << (static_cast<uint32_t>(x.varnum()) % m_numLiveVars);
}

std::string
protoConverter::FunctionToString(Function const &input)
{
  visit(input);
  return m_output.str();
}
