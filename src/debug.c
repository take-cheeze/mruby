...
get_file(mrb_irep_debug_info *info, uint32_t pc)
{
  ...
  mrb_assert(
      (*ret)->start_pos <= pc &&
      pc < (((ret + 1 - info->files) < info->flen) ? (*(ret + 1))->start_pos : info->pc_count));
...
mrb_debug_info_append_file(mrb_state *mrb, mrb_irep_debug_info *d, const char *filename,
                           uint16_t *lines, uint32_t start_pos, uint32_t end_pos)
{
  ...
  if (d->flen > 0) {
    ...
    if (strcmp(filename, fn) == 0) return NULL;
  }

  f = (mrb_irep_debug_info_file *)mrb_malloc(mrb, sizeof(*f));
  d->files =
      (mrb_irep_debug_info_file **)(d->files ? mrb_realloc(mrb, d->files,
  ...
  case mrb_debug_line_flat_map: {
    ...
    f->lines.flat_map =
        (mrb_irep_debug_info_line *)mrb_malloc(mrb, sizeof(mrb_irep_debug_info_line) * 1);
    f->line_entry_count = 0;
    for (i = 0; i < file_pc_count; ++i) {
      if (lines[start_pos + i] == prev_line) {
        continue;
      }

      f->lines.flat_map = (mrb_irep_debug_info_line *)mrb_realloc(
          mrb, f->lines.flat_map, sizeof(mrb_irep_debug_info_line) * (f->line_entry_count + 1));
