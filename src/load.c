...
read_irep_record_1(mrb_state *mrb, const uint8_t *bin, size_t *len, uint8_t flags)
{
  ...
  if (irep->ilen > 0) {
    ...
    if ((flags & FLAG_SRC_MALLOC) == 0 && (flags & FLAG_BYTEORDER_NATIVE)) {
...
static int
read_debug_record(mrb_state *mrb, const uint8_t *start, mrb_irep *irep, size_t *record_len,
                  const mrb_sym *filenames, size_t filenames_len)
{
  ...
  irep->debug_info->files = (mrb_irep_debug_info_file **)mrb_malloc(
      mrb, sizeof(mrb_irep_debug_info *) * irep->debug_info->flen);
  bin += sizeof(uint16_t);
  ...
  for (f_idx = 0; f_idx < irep->debug_info->flen; ++f_idx) {
    ...
    case mrb_debug_line_ary: {
      ...
      file->lines.ary =
          (uint16_t *)mrb_malloc(mrb, sizeof(uint16_t) * (size_t)(file->line_entry_count));
...
static int
read_lv_record(mrb_state *mrb, const uint8_t *start, mrb_irep *irep, size_t *record_len,
               mrb_sym const *syms, uint32_t syms_len)
{
  const uint8_t *bin = start;
  ptrdiff_t diff;
  int i;
...
static int
read_binary_header(const uint8_t *bin, size_t bufsize, size_t *bin_size, uint16_t *crc,
                   uint8_t *flags)
{
  ...
  } else if (memcmp(header->binary_ident, RITE_BINARY_IDENT_LIL, sizeof(header->binary_ident)) ==
             0) {
    if (bigendian_p())
...
read_irep(mrb_state *mrb, const uint8_t *bin, size_t bufsize, uint8_t flags)
{
  ...
  do {
    ...
    if (memcmp(section_header->section_ident, RITE_SECTION_IREP_IDENT,
               sizeof(section_header->section_ident)) == 0) {
      irep = read_section_irep(mrb, bin, flags);
      if (!irep) return NULL;
    } else if (memcmp(section_header->section_ident, RITE_SECTION_DEBUG_IDENT,
                      sizeof(section_header->section_ident)) == 0) {
    ...
    } else if (memcmp(section_header->section_ident, RITE_SECTION_LV_IDENT,
                      sizeof(section_header->section_ident)) == 0) {
      if (!irep) return NULL;
  ...
  } while (memcmp(section_header->section_ident, RITE_BINARY_EOF,
                  sizeof(section_header->section_ident)) != 0);
