...
#define NATIVE_DEFINE_TYPE_FUNC(t)                               \
  static mrb_value native_s_##t(mrb_state *mrb, mrb_value klass) \
  {                                                              \
...
static const struct mrb_data_type native_data_type = {"TestVFormat::Native", native_free};
...
native_initialize(mrb_state *mrb, mrb_value self)
{
  ...
  switch (data.type) {
  case ARG_c:
    data.c = RSTRING_PTR(obj)[0];
    break;
  case ARG_d:
    data.d = (int)mrb_fixnum(obj);
    break;
  case ARG_f:
  ...
  case ARG_n:
    ...
    break;
  case ARG_s:
    data.s = (char *)mrb_malloc(mrb, RSTRING_LEN(obj) + 1);
    memcpy(data.s, RSTRING_PTR(obj), RSTRING_LEN(obj));
    data.s[RSTRING_LEN(obj)] = '\0';
    break;
  case ARG_C:
    data.C = mrb_class_ptr(obj);
    break;
  default:
    mrb_raise(mrb, E_ARGUMENT_ERROR, "unknown type");
  }
...
static VFArgument *
arg_from_obj(mrb_state *mrb, mrb_value obj, struct RClass *native_class, VFArgument *vf_arg)
{
...
#define VF_FORMAT_INIT(klass)                                                                \
  struct RClass *vf_native_class = mrb_class_get_under(mrb, mrb_class_ptr(klass), "Native"); \
  VFArgument vf_args[2];

#define VF_ARG(args, idx) arg_from_obj(mrb, args[idx], vf_native_class, &vf_args[idx])

#define VF_FORMAT0(fmt)       mrb_format(mrb, fmt);
#define VF_FORMAT1(fmt, args) (VF_ARG(args, 0), VF_FORMAT_TYPED(fmt, 1, vf_args, NULL))
#define VF_FORMAT2(fmt, args)                                                           \
  (VF_ARG(args, 0), VF_ARG(args, 1), VF_FORMAT2_COND_EXPR(fmt, vf_args, vf_args + 1, c) \
...
#define VF_FORMAT_TYPED(fmt, n_arg, type_a, v1)                                      \
  VF_FORMAT_TYPED_COND_EXPR(fmt, n_arg, type_a, v1, c)                               \
      ...
      : VF_FORMAT_TYPED_COND_EXPR(fmt, n_arg, type_a, v1, C)                         \
      : VF_FORMAT_TYPED_COND_EXPR(fmt, n_arg, type_a, v1, v) : mrb_nil_value() /* not reached */
#define VF_FORMAT_TYPED_COND_EXPR(fmt, n_arg, type_a, v1, t)                 \
  (type_a)->type == ARG_##t ? n_arg == 1 ? mrb_format(mrb, fmt, (type_a)->t) \
                                         : mrb_format(mrb, fmt, v1, (type_a)->t)
...
vf_s_format(mrb_state *mrb, mrb_value klass)
{
  ...
  switch (argc) {
  case 1:
    return VF_FORMAT0(fmt);
  case 2:
    return VF_FORMAT1(fmt, args);
  case 3:
    return VF_FORMAT2(fmt, args);
  default:
