...
static int32_t
check_method_breakpoint(mrb_state *mrb, mrb_irep *irep, mrb_code const *pc, mrb_value *regs)
{
  struct RClass* c;
  mrb_sym sym;
  int32_t bpno;
...
static void
mrb_code_fetch_hook(mrb_state *mrb, mrb_irep *irep, mrb_code const *pc, mrb_value *regs)
{
  const char *file;
  int32_t line;
  int32_t bpno;
