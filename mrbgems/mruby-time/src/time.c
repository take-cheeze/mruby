...
timegm(struct tm *tm)
{
  static const unsigned int ndays[2][12] = {{31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31},
  ...
  for (i = 0; i < tm->tm_mon; ++i) r += nday[i] * 24 * 60 * 60;
...
#define MRB_TIME_MIN    (MRB_TIME_T_UINT ? 0 : (sizeof(time_t) <= 4 ? INT32_MIN : INT64_MIN))
#define MRB_TIME_MAX                                                 \
...
static mrb_value
mrb_time_make_time(mrb_state *mrb, struct RClass *c, time_t sec, time_t usec,
                   enum mrb_timezone timezone)
{
...
static mrb_value
mrb_time_make(mrb_state *mrb, struct RClass *c, mrb_value sec, mrb_value usec,
              enum mrb_timezone timezone)
{
...
static struct mrb_time *
time_mktime(mrb_state *mrb, mrb_int ayear, mrb_int amonth, mrb_int aday, mrb_int ahour,
            mrb_int amin, mrb_int asec, mrb_int ausec, enum mrb_timezone timezone)
{
  time_t nowsecs;
  struct tm nowtime = {0};
  ...
  if (nowtime.tm_mon < 0 || nowtime.tm_mon > 11 || nowtime.tm_mday < 1 || nowtime.tm_mday > 31 ||
      nowtime.tm_hour < 0 || nowtime.tm_hour > 24 ||
      (nowtime.tm_hour == 24 && (nowtime.tm_min > 0 || nowtime.tm_sec > 0)) || nowtime.tm_min < 0 ||
      nowtime.tm_min > 59 || nowtime.tm_sec < 0 || nowtime.tm_sec > 60)
...
mrb_time_gm(mrb_state *mrb, mrb_value self)
{
  ...
  mrb_get_args(mrb, "i|iiiiii", &ayear, &amonth, &aday, &ahour, &amin, &asec, &ausec);
  return mrb_time_wrap(
      mrb, mrb_class_ptr(self),
...
mrb_time_plus(mrb_state *mrb, mrb_value self)
{
  ...
  return mrb_time_make_time(mrb, mrb_obj_class(mrb, self), tm->sec + sec, tm->usec + usec,
                            tm->timezone);
}
...
#ifndef MRB_WITHOUT_FLOAT
    mrb_float f;
    f = (mrb_sec)(tm->sec - tm2->sec) + (mrb_sec)(tm->usec - tm2->usec) / 1.0e6;
...
mrb_time_zone(mrb_state *mrb, mrb_value self)
{
  ...
  return mrb_str_new_static(mrb, timezone_names[tm->timezone].name,
...
#  endif
  ...
  len = snprintf(buf, sizeof(buf), "%s %s %2d %02d:%02d:%02d %.4d", wday_names[d->tm_wday],
                 mon_names[d->tm_mon], d->tm_mday, d->tm_hour, d->tm_min, d->tm_sec,
                 d->tm_year + 1900);
#endif
...
mrb_time_initialize(mrb_state *mrb, mrb_value self)
{
  ...
  n = mrb_get_args(mrb, "|iiiiiii", &ayear, &amonth, &aday, &ahour, &amin, &asec, &ausec);
...
mrb_mruby_time_gem_init(mrb_state *mrb)
{
  ...
  mrb_define_method(mrb, tc, "initialize_copy", mrb_time_initialize_copy,
                    MRB_ARGS_REQ(1)); /* 15.2.19.7.17 */
